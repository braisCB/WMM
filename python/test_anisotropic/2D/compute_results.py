    ...
    for interpolation_technique in interpolation_techniques:
        for search_mode in search_modes:
            ...
            for _ in range(niter):
                # info[label]['surface'] = mm.oum2D(gradient, initial_points, h, 7, search_mode)
                info[label]['surface'] = mm.ani_wmm2D(gradient, initial_points, h, interpolation_technique, search_mode)
...
def compute_oum_results(gradient, ground_truth, initial_points, niter=10, h=1, gamma=6,
                        search_modes=('gradient', 'hopf_lax', 'golden_search',)):

    info = {}

    for search_mode in search_modes:
        label = 'oum_' + search_mode
        print(label)
        info[label] = {}
        start_time = time.time()
        for _ in range(niter):
            info[label]['surface'] = mm.oum2D(gradient, initial_points, h, gamma, search_mode)
        info[label]['time'] = (time.time() - start_time) / niter
        info[label]['metrics'] = {
            'l1': np.abs(info[label]['surface'] - ground_truth).mean(),
            'l2': np.square(info[label]['surface'] - ground_truth).mean(),
...
def aug_compute_results(gradient, ground_truth, initial_points, niter=10, inner_nodes=3, h=1,
    ...
    for interpolation_technique in interpolation_techniques:
        for search_mode in search_modes:
            ...
            start_time = time.time()
            for _ in range(niter):
                info[label]['surface'] = mm.aug_ani_wmm2D(gradient, initial_points, h, inner_nodes, interpolation_technique, search_mode)
            info[label]['time'] = (time.time() - start_time) / niter
            info[label]['metrics'] = {
                'l1': np.abs(info[label]['surface'] - ground_truth).mean(),
                'l2': np.square(info[label]['surface'] - ground_truth).mean(),
