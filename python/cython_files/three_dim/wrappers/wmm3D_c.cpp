...
void wmm3D_c(
    ...
    switch (interp) {
        case wmm3D::I_BILINEAR:
            wmm3D::wmm3DIsoSurface3D<0>(image_grid, node_initials, hs, interp, search, u_surface_grid);
            break;
        case wmm3D::I_QUADATRIC:
            wmm3D::wmm3DIsoSurface3D<4>(image_grid, node_initials, hs, interp, search, u_surface_grid);
            break;
        case wmm3D::I_CUBIC:
            wmm3D::wmm3DIsoSurface3D<9>(image_grid, node_initials, hs, interp, search, u_surface_grid);
            break;
        default: //I_SPLINE, I_HERMITE and I_PCHIP
            wmm3D::wmm3DIsoSurface3D<11>(image_grid, node_initials, hs, interp, search, u_surface_grid);
    }
