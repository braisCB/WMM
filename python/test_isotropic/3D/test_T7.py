    ...
    for i in range(tam):
        for j in range(tam):
            for k in range(tam):
                ...
                ground_truth[i,j,k] = np.square(pos[0]) / 25 + np.square(pos[1]) / 16 + np.square(pos[2]) / 36
                gradient[i, j, k] = 2 * pos / np.array([25, 16, 36])
