...
namespace wmm3D {
    ...
    template<int N> double GetInterpValue(Grid3 &image, wmm3_<double, N> &wave, int ndir,
        ...
        double a_00 = image.at(neigh, 0), a_01 = image.at(neigh, 1), a_02 = image.at(neigh, 2);
        double a_10 = image.at(neigh, 3), a_11 = image.at(neigh, 4), a_12 = image.at(neigh, 5);
        double a_20 = image.at(neigh, 6), a_21 = image.at(neigh, 7), a_22 = image.at(neigh, 8);
        ...
        if ((vt < wave.v && vt < wave.planes[ndir].v[0] && vt < wave.planes[ndir].v[1] && vt < wave.planes[ndir].v[2] &&
            ...
            Node3D diff = h * (neigh - plane_posD);
            Node3D a_diff(a_00 * diff.x + a_01 * diff.y + a_02 * diff.z,
                          a_10 * diff.x + a_11 * diff.y + a_12 * diff.z,
                          a_20 * diff.x + a_21 * diff.y + a_22 * diff.z);
            double dist = sqrt(diff.x * a_diff.x + diff.y * a_diff.y + diff.z * a_diff.z);
            value = vt + dist;
        }
    ...
    template<int N> double GetVal3D(Grid3 &image, Grid3 &u_surface, wmm3_<double, N> &wave,
                                    Node3 &neigh, Node3D &h, int interp, int mode, int &epsilon_best) {
        ...
        std::pair<double, double> epsilon(0., 0.);

        epsilon_best = epsilon;

        //Node3D diff = h * (neigh - wave.p);
        //Node3D a = diff / norm(diff);
        double val = GetInterpValue(image, wave, -1, interp, epsilon, neigh, h), new_val;
        // y0 + norm(diff) / sqrt(1.0 + (fn.x*a.x + fn.y*a.y + fn.z*a.z)*(fn.x*a.x + fn.y*a.y + fn.z*a.z));
            ...
            for (int i=0; i < 3; i++) {
                for (int j=0; j < 3; j++) {
                    for (int k=0; k < 3; k++) {
                        ...
                        dir_surface.at(new_w.p, 0) = d.x;
                        dir_surface.at(new_w.p, 1) = d.y;
                        dir_surface.at(new_w.p, 2) = d.z;
                    }
                }
            }
    ...
    template<int wmm_degree> Grid3 wmm3DAniSurface3D(Grid3 &image, std::vector<Node3> &initials,
        ...
        Grid3 dir_surface = Grid3(0, image.rows, image.cols, image.channels, 3);
        wmm3DAniSurface3D<wmm_degree>(image, initials, h, interp, mode, u_surface, dir_surface);
        return u_surface;
    }
