...
namespace wmm3D {
    ...
    template<int N> double GetInterpValue(Grid3 &image, wmm3_<double, N> &wave, int ndir,
        ...
        Node3D f0_3 = Node3D(image.at(wave.p, 0), image.at(wave.p, 1), image.at(wave.p, 2));
        Node3D f1_3 = Node3D(image.at(p1, 0), image.at(p1, 1), image.at(p1, 2));
        Node3D f2_3 = Node3D(image.at(p2, 0), image.at(p2, 1), image.at(p2, 2));
        Node3D f3_3 = Node3D(image.at(p3, 0), image.at(p3, 1), image.at(p3, 2));

        double value = MAX_VAL, ft, vt;

        if (interp == I_BILINEAR) {
            ft = (1.0 - epsilon.first)*(1.0 - epsilon.second)*norm(f0_3) +
                 epsilon.first*(1.0 - epsilon.second)*norm(f1_3) +
                 (1.0 - epsilon.first)*epsilon.second*norm(f2_3) +
                 epsilon.first*epsilon.second*norm(f3_3);
            ...
            double m[2], fm[2], v[3], f[3], f0, f1, y0, y1;
            ...
            double m[2], fm[2], v[3], f[3], f0, f1, y0, y1, d0, d1, fd0, fd1;
            double t = epsilon.first, t_2 = t*t, t_3 = t_2*t;
            int pos = (int) wave.planes[ndir].m[0];
            for (int p=0; p<3; p++) {
                ...
                if (pos == 0) {
                    ...
                    fm[0] = (3.0 * fd0 - fd1) / 2.0;
                    if ((fm[0] * fd0) <= 0.0)
                        fm[0] = 0.0;
                    else if (((fd1 * fd0) <= 0.0) && (fabs(fm[0]) > fabs(3.0 * fd0)))
                        fm[0] = 3.0 * fd0;
                    fm[1] = (fd0 * fd1 <= 0.0) ? 0.0 : 2.0 * fd0 * fd1 / (fd0 + fd1);
                }
                else {
                    ...
                    fm[0] = (fd0 * fd1 <= 0.0) ? 0.0 : 2.0 * fd0 * fd1 / (fd0 + fd1);
                    fm[1] = (3.0 * fd1 - fd0) / 2.0;
                    if ((fm[1] * fd1) <= 0.0)
                        fm[1] = 0.0;
                    else if (((fd0 * fd1) <= 0.0) && (fabs(fm[1]) > fabs(3.0 * fd1)))
                        fm[1] = 3.0 * fd1;
                }
                y0 = wave.planes[ndir].m[2 + 3*p + pos];
                y1 = wave.planes[ndir].m[2 + 3*p + 1 + pos];
            ...
            for (int p=0; p<3; p++) {
                if (pos == 0) {
                    ...
                    fm[0] = 0.0;
                    fm[1] = 6.0*(wave.planes[ndir].fm[2 + 3*p] - 2.0*wave.planes[ndir].fm[2 + 3*p + 1] + wave.planes[ndir].fm[2 + 3*p + 2])/4.0;
                }
                else {
                    m[0] = 6.0*(wave.planes[ndir].m[2 + 3*p] - 2.0*wave.planes[ndir].m[2 + 3*p + 1] + wave.planes[ndir].m[2 + 3*p + 2])/4.0;
                    m[1] = 0.0;
                    fm[0] = 6.0*(wave.planes[ndir].fm[2 + 3*p] - 2.0*wave.planes[ndir].fm[2 + 3*p + 1] + wave.planes[ndir].fm[2 + 3*p + 2])/4.0;
                    fm[1] = 0.0;
                }
                y0 = wave.planes[ndir].m[2 + 3*p + pos];
                y1 = wave.planes[ndir].m[2 + 3*p + 1 + pos];
                f0 = wave.planes[ndir].fm[2 + 3*p + pos];
                f1 = wave.planes[ndir].fm[2 + 3*p + 1 + pos];
                f[p] = f0 + t*(-fm[1]/6.0 - fm[0]/3.0 + f1 - f0 + t*(fm[0]/2.0 + t*(fm[1] - fm[0])/6.0));
            ...
            f0 = f[pos];
            f1 = f[1 + pos];
            ft = f0 + t*(-fm[1]/6.0 - fm[0]/3.0 + f1 - f0 + t*(fm[0]/2.0 + t*(fm[1] - fm[0])/6.0));
        ...
        if ((vt < wave.v && vt < wave.planes[ndir].v[0] && vt < wave.planes[ndir].v[1] && vt < wave.planes[ndir].v[2] &&
            ...
            interp != I_PCHIP && interp != I_BILINEAR) ||
            (ft < norm(f0_3) && ft < norm(f1_3) && ft < norm(f2_3) && ft < norm(f3_3) &&
            interp != I_PCHIP && interp != I_BILINEAR)) ||
            (ft > norm(f0_3) && ft > norm(f1_3) && ft > norm(f2_3) && ft > norm(f3_3) &&
            interp != I_PCHIP && interp != I_BILINEAR)) {
            if (interp == I_HERMITE)
            ...
            double fti = GetIntegralValue(image, plane_posD, neighD, ft, norm(fn), interp, 1);
    ...
    template<int N> double GetVal3D(Grid3 &image, Grid3 &u_surface, wmm3_<double, N> &wave,
        ...
        Node3D diff(h.x * (neigh.x - wave.p.x), h.y * (neigh.y - wave.p.y), h.z * (neigh.z - wave.p.z));
        Node3D wave_p(wave.p.x, wave.p.y, wave.p.z), neighD(neigh.x, neigh.y, neigh.z);
        double fti = GetIntegralValue(image, wave_p, neighD, norm(f0), norm(fn), interp, 1);
        double val = y0 + norm(diff) * fti; //(norm(f0) + norm(fn)) / 2.0;
    ...
    template<int wmm_degree> void wmm3DIsoSurface3D(Grid3 &image, std::vector<Node3> &initials,
    ...
    template<int wmm_degree> Grid3 wmm3DIsoSurface3D(Grid3 &image, std::vector<Node3> &initials,
                                                            Node3D &h, int interp, int mode) {
        Grid3 u_surface = Grid3(MAX_VAL, image.rows, image.cols, image.channels, 1);
        wmm3DIsoSurface3D<wmm_degree>(image, initials, h, interp, mode, u_surface);
        return u_surface;
    }
