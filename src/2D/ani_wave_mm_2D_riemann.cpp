...
namespace wmm {
    ...
    template<int N> double GetInterpValue(Grid &image, Wmm_<double, N> &wave, NodeD &dp, NodeD &dd, NodeD &dn, NodeD &h,
        ...
        Node neigh((int) dn.y, (int) dn.x);
        double value, y0 = wave.v[0], y1 = wave.v[side + 1];
        double a_00 = image.at(neigh, 0), a_01 = image.at(neigh, 1), a_10 = image.at(neigh, 2), a_11 = image.at(neigh, 3);
    ...
    template<int N> double GetVal2D(Grid &image, Grid &u_surface, Wmm_<double, N> &wave, Node &neigh, NodeD &h, int interp, int mode) {
        ...
        double val = MAX_VAL;
        if (wave.dir < 0) {
//            double v_y = image.at(neigh, 0), v_x = image.at(neigh, 1), e_1 = image.at(neigh, 2), e_2 = image.at(neigh, 3);
//            double a_00 = e_1 * v_y * v_y + e_2 * v_x * v_x, a_01 = (e_2 - e_1) * v_y * v_x;
//            double a_10 = a_01, a_11 = e_2 * v_y * v_y + e_1 * v_x * v_x;
            double a_00 = image.at(neigh, 0), a_01 = image.at(neigh, 1), a_10 = image.at(neigh, 2), a_11 = image.at(neigh, 3);
            NodeD diff(h.y * (neigh.y - wave.p.y), h.x * (neigh.x - wave.p.x));
            NodeD a_diff(a_00 * diff.y + a_01 * diff.x, a_10 * diff.y + a_11 * diff.x);
            double dist = sqrt(diff.y * a_diff.y + diff.x * a_diff.x);
            val = y0 + dist; //(norm(f0) + norm(fn)) / 2.0;
        }
        else {
                ...
                for (i = 0; i < 8; i++) {
                    if (valcenter[i] == MAX_VAL) {
                        if (valcenter[(i + 2) % 8] == MAX_VAL && valcenter[(i + 6) % 8] == MAX_VAL)
                            valcenter[i] = 2.0 * winner.v[0] - valcenter[(i + 4) % 8];
                        else if (valcenter[(i + 2) % 8] == MAX_VAL)
                            valcenter[i] = 2.0*valcenter[(i + 7) % 8] - valcenter[(i + 6) % 8];
                        else
                            valcenter[i] = 2.0*valcenter[(i + 1) % 8] - valcenter[(i + 2) % 8];
                    }
                }
                i = winner.dir;
