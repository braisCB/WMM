...
namespace wmm {
    ...
    double GetInterpValue(Grid &image, AugWmm &wave, int ndir,
        ...
        value = vt + norm(diff) * sqrt(1.0 + (fn.x*a.x + fn.y*a.y)*(fn.x*a.x + fn.y*a.y));
    ...
    double GetEpsilonGradient(AugWmm &wave, int ndir, NodeD &neigh, NodeD &c, NodeD &h) {
    ...
    double GetVal2D(Grid &image, Grid &u_surface, AugWmm &wave, NodeD &neigh, NodeD &fn, NodeD &h, int interp, int mode) {
        ...
        NodeD a = diff / norm(diff);
        double val = y0 + norm(diff) * sqrt(1.0 + (fn.x*a.x + fn.y*a.y)*(fn.x*a.x + fn.y*a.y));
