...
namespace wmm {
    ...
    double StencilS2(wmm::Grid_<unsigned char> &state, wmm::Grid &u_surface, wmm::Node &neigh, wmm::NodeD &h, double F, int order) {
    ...
    double GetVal2D(wmm::Grid_<unsigned char> &state, wmm::Grid &u_surface, wmm::Node &neigh, wmm::NodeD &h, double F, int order) {
        ...
        double stencilS2_val = wmm::StencilS2(state, u_surface, neigh, h, F, order);
        return std::min(stencilS1_val, stencilS2_val);

    }


    void MSFMIsoSurface2D(wmm::Grid &image, std::vector<wmm::Node> &initials, wmm::NodeD &h, int order, wmm::Grid &u_surface) {
    ...
    wmm::Grid MSFMIsoSurface2D(wmm::Grid &image, std::vector<wmm::Node> &initials, wmm::NodeD &h, int order) {
        wmm::Grid u_surface = wmm::Grid(wmm::MAX_VAL, image.rows, image.cols, 1);
        MSFMIsoSurface2D(image, initials, h, order, u_surface);
        return u_surface;
    }
