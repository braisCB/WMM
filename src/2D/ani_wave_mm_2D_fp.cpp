...
namespace wmm {
    ...
    template<int N> double GetInterpValue(Grid &image, Wmm_<double, N> &wave, NodeD &dp, NodeD &dd, NodeD &dn, NodeD &h,
                                          Node3D &f0, Node3D &f1, Node3D &c, double epsilon, int interp,
                                          int side) {
        ...
        //double F = c.z * sqrt(((1.0 + c.x * c.x) * a.x * a.x + (1.0 + c.y * c.y) * a.y * a.y - 2.0 * c.x * c.y * a.x * a.y) /
        //                      ((1.0 + c.x * c.x + c.y * c.y) * (a.x * a.x + a.y * a.y)));
        value += norm(diff) * sqrt(1.0 + (c.x*a.x + c.y*a.y)*(c.x*a.x + c.y*a.y)) / c.z;
    ...
    double GetEpsilonGradient(NodeD dd, NodeD dp, NodeD dn, Node3D &c) {
    ...
    template<int N> double GetEpsilonGoldenSearch(Grid &image, Wmm_<double, N> &wave, NodeD &dn, NodeD &dd, NodeD &dp,
                                                  NodeD &h, Node3D &f0, Node3D &f1, Node3D &fn, int interp, int side) {
    ...
    template<int N> double GetVal2D(Grid &image, Grid &u_surface, Wmm_<double, N> &wave, Node &neigh, NodeD &h, int interp, int mode) {
        ...
        Node3D f0, fn;
        if (image.channels == 3) {
            f0 = Node3D(image.at(wave.p, 0), image.at(wave.p, 1), image.at(wave.p, 2)), fn = Node3D(image.at(neigh, 0), image.at(neigh, 1), image.at(neigh, 2));
        }
        else {
            f0 = Node3D(image.at(wave.p, 0), image.at(wave.p, 1), 1.0), fn = Node3D(image.at(neigh, 0), image.at(neigh, 1), 1.0);
        }
        double y0 = wave.v[0];
        ...
        if (wave.dir < 0) {
            ...
            NodeD a = diff / norm(diff);
            NodeD dirs(0.0, 0.0);
            val = y0 + norm(diff) * sqrt(1.0 + (c.x*a.x + c.y*a.y)*(c.x*a.x + c.y*a.y)) / c.z;
        }
        else {
            ...
            if (u_surface.contains(p)) {
                ...
                Node3D f1(image.at(p, 0), image.at(p, 1), image.at(p, 2));
