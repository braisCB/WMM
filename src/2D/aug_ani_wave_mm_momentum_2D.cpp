...
namespace wmm {
    ...
    std::pair<double, wmm::NodeD> GetInterpValue(wmm::AugWmm &wave, wmm::NodeD &dp, wmm::NodeD &dd, wmm::NodeD &dn,
                          wmm::Node3D &f0, wmm::Node3D &f1, double fn, double epsilon, int interp, int side, int gamma,
                          double alpha, double momentum) {

        // f0, f1 and f0 must be in polar coordinates
        
        double value = GetInterpV(wave, epsilon, interp, side, gamma);
    ...
    double GetEpsilonGoldenSearch(wmm::AugWmm &wave, wmm::NodeD &dn, wmm::NodeD &dd, wmm::NodeD &dp,
                                  ...
                                  double interp, int side, int gamma, double alpha, double momentum) {
        ...
        double epsilon;
        double f_a = GetInterpValue(wave, dp, dd, dn, f0, f1, fn, 0.0, interp, side, gamma, alpha, momentum).first;
        double f_b = GetInterpValue(wave, dp, dd, dn, f0, f1, fn, 1.0, interp, side, gamma, alpha, momentum).first;
        ...
        f_x1 = GetInterpValue(wave, dp, dd, dn, f0, f1, fn, x1, interp, side, gamma, alpha, momentum).first;
        f_x2 = GetInterpValue(wave, dp, dd, dn, f0, f1, fn, x2, interp, side, gamma, alpha, momentum).first;
    ...
    std::pair<double, wmm::NodeD> GetVal2D(wmm::Grid &image, wmm::Grid &f_surface, wmm::Grid &u_surface, wmm::AugWmm &wave, 
            wmm::NodeD &neigh, double fn, wmm::NodeD &h, int interp, int mode,int gamma, double alpha, double momentum) {

        wmm::NodeD f0(f_surface.at(wave.p, 0), f_surface.at(wave.p, 1));
        double y0 = wave.v[0], step = 1.0 / (double) gamma;
        int i;
        
        
        wmm::Node3D f0_polar = wmm::to_polar(f0);
        
        std::pair<double, wmm::NodeD> val = std::pair<double, wmm::NodeD>(wmm::MAX_VAL, wmm::NodeD(0.0, 0.0));
        if (wave.dir < 0) {
            ...
            c = 1.0 / wmm::norm(c) * c;
            val = std::pair<double, wmm::NodeD>(y0 + wmm::norm(diff) * fn, c);
        }
        else {

            wmm::Node p(wave.p.y + wmm::yarray[(wave.dir + 1) % 8] - wmm::yarray[wave.dir],
                    wave.p.x + wmm::xarray[(wave.dir + 1) % 8] - wmm::xarray[wave.dir]);
            std::pair<double, wmm::NodeD> res1 = std::pair<double, wmm::NodeD>(wmm::MAX_VAL, wmm::NodeD(0.0, 0.0)), res_aux;
            ...
            if (u_surface.contains(p)) {
                ...
                wmm::NodeD f1(f_surface.at(p, 0), f_surface.at(p, 1)), dp_aux, dd_aux, f_aux;
                ...
                switch (mode) {
                    case wmm::M_GRADIENT:
                        ...
                        for (i=1; i<=gamma; i++) {
                            ...
                            f_aux = wmm::to_cartesian((i-1)*step*f1_polar + (1.0 - (i-1)*step)*f0_polar);
                            epsilon_aux = (i - 1.0 + GetEpsilonGradient(dd, dp, dn, f_aux))*step;
                            res_aux = GetInterpValue(wave, dp, dd, dn, f0_polar, f1_polar, fn, epsilon_aux, interp, wmm::S_LEFT, gamma, alpha, momentum);
                            if (res_aux.first < res1.first) {
                                res1 = res_aux;
                                epsilon = epsilon_aux;
                            }
                            y0 = y1;
                    ...
                    case wmm::M_HOPFLAX:
                        ...
                        for (i=1; i<=gamma; i++) {
                            ...
                            res_aux = GetInterpValue(wave, dp, dd, dn, f0_polar, f1_polar, fn, epsilon_aux, interp, wmm::S_RIGHT, gamma, alpha, momentum);
                            if (res_aux.first < res1.first) {
                                res1 = res_aux;
                                epsilon = epsilon_aux;
                            }
                    ...
                    default: //M_GOLDENSEARCH
                        epsilon = GetEpsilonGoldenSearch(wave, dn, dd, dp, f0_polar, f1_polar, fn, interp, wmm::S_RIGHT, gamma, alpha, momentum);
                }

                res1 = GetInterpValue(wave, dp, dd, dn, f0_polar, f1_polar, fn, epsilon, interp, wmm::S_RIGHT, gamma, alpha, momentum);
            }

            p = wmm::Node(wave.p.y + wmm::yarray[(wave.dir + 7) % 8] - wmm::yarray[wave.dir],
                    wave.p.x + wmm::xarray[(wave.dir + 7) % 8] - wmm::xarray[wave.dir]);
            std::pair<double, wmm::NodeD> res2 = std::pair<double, wmm::NodeD>(wmm::MAX_VAL, wmm::NodeD(0.0, 0.0));
            ...
            if (u_surface.contains(p)) {
                ...
                switch (mode) {
                    ...
                    case wmm::M_HOPFLAX:
                        ...
                        break;
                    default: //M_GOLDENSEARCH
                        epsilon = GetEpsilonGoldenSearch(wave, dn, dd, dp, f0_polar, f1_polar, fn, interp, wmm::S_LEFT, gamma, alpha, momentum);
                }

                res2 = GetInterpValue(wave, dp, dd, dn, f0_polar, f1_polar, fn, epsilon, interp, wmm::S_LEFT, gamma, alpha, momentum);
    ...
    std::pair<wmm::Grid, wmm::Grid> WmmAugAniSurfaceMomentum2D(wmm::Grid &image, std::vector<wmm::Node> &initials, wmm::NodeD &h, int interp, int mode, 
            int N, int M, int gamma, double alpha, double momentum) {

        bool isnewpos[8];
        double valcenter[8];
        double imcenter_value[8];
        double imcenter_module[8];
        double imcenter_cos[8];
        double imcenter_sin[8], minimo, step = 1.0/gamma;
        double vinterp[2*gamma + 1], ival;
        ...
        wmm::Grid f_surface = wmm::Grid(image.rows, image.cols, 2);
        ...
        int key, i, j;
        wmm::AugWmm winner, new_w, father;
        wmm::Node neigh;
        wmm::NodeD neighD, neighD1, fn, d1, d2, neigh_aux, c;
        wmm::Node3D polar0, polar1, polar2;
        ...
        while (!trial_set.empty()) {
            ...
            father = trial_set_it->second;
            if (father.child_dir == -1)
                winner = wmm::AugWmm(&father);
            else {
                // father neighbor computation
                for (i = 0; i < 8; i++) {
                    neigh = father.p + wmm::Node(wmm::yarray[i], wmm::xarray[i]);
                    isnewpos[i] = false;
                    if (u_surface.contains(neigh)) {
                        valcenter[i] = u_surface.at(neigh);
                        imcenter_value[i] = image.at(neigh);
                        fn = wmm::NodeD(f_surface.at(neigh, 0), f_surface.at(neigh, 1));
                    }
                    else {
                        valcenter[i] = father.v[0];
                        imcenter_value[i] = image.at(father.p);
                        fn = wmm::NodeD(f_surface.at(father.p, 0), f_surface.at(father.p, 1));
                    }
                ...
                winner = wmm::AugWmm(N, M, gamma);
                winner.dir = father.child_dir;
                winner.child_dir = -1;
                neigh = father.p + wmm::Node(wmm::yarray[winner.dir], wmm::xarray[winner.dir]);
                winner.p = neigh;
                winner.v[0] = valcenter[winner.dir];
                winner.v[gamma] = valcenter[(winner.dir + 1)%8];
                winner.v[2*gamma] = valcenter[(winner.dir + 7)%8];
                i = winner.dir;
                ...
                neighD = wmm::NodeD(neigh.y, neigh.x);
                ...
                for (j = 1; j < gamma; j++) {
                    ...
                    if (u_surface.contains(neigh_aux))
                        vinterp[j] = GetVal2D(image, f_surface, u_surface, father, neigh_aux, imcenter_value[i], h, interp, mode, gamma, alpha, momentum).first;
                    else
                ...
                vinterp[2*gamma] = valcenter[(i+7)%8];
                ...
                //std::cout << "hijo (" << winner.p.y << ", " << winner.p.x << ", " << winner.dir << ") v = " << winner.v[0] << std::endl;
            }
            ...
            if (mapa_trial_it == mapa_trial.end()) {
                ...
                return std::pair<wmm::Grid, wmm::Grid>(u_surface, f_surface);
            }
            ...
            for (i = 0; i < 8; i++) {
                ...
                if (u_surface.contains(neigh)) {
                    ...
                    imcenter_value[i] = image.at(winner.p);
                }
                if (u_surface.contains(neigh) && state.at(neigh) != wmm::P_ALIVE) {
                    neighD = wmm::NodeD(winner.p.y + wmm::yarray[i], winner.p.x + wmm::xarray[i]);
                    std::pair<double, wmm::NodeD> val_neigh = GetVal2D(image, f_surface, u_surface, winner, neighD, imcenter_value[i], h, interp, mode, gamma, alpha, momentum);
                    if (val_neigh.first < valcenter[i]) {
                        valcenter[i] = val_neigh.first;
                        f_surface.at(neigh, 0) = val_neigh.second.y;
                        f_surface.at(neigh, 1) = val_neigh.second.x;
                        isnewpos[i] = true;
                    }
                }
            }
