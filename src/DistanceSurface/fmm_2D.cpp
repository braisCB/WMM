...
namespace wmm {
    ...
    std::pair<double, double> GetVal2D(wmm::Grid_<unsigned char> &state, wmm::Grid &u_surface, wmm::Node &neigh, wmm::NodeD &h, double F, int order) {
        ...
        double stencil = wmm::StencilS1(state, u_surface, neigh, h, F, order, 0);
        double distance_stencil = wmm::StencilS1(state, u_surface, neigh, h, 1.0, order, 1);
        return std::pair<double, double>(stencil, distance_stencil);

    }


    void FMMIsoSurface2D(wmm::Grid &image, std::vector<wmm::Node> &initials, std::vector<wmm::Node> &finals,
